...
impl OmniPaxosDurability {
    ...
    fn decode_log_entry_decided(
        log_entry: utilLogEntry<OmniLogEntry>,
    ) -> Option<(TxOffset, TxData)> {
        match log_entry {
...
impl DurabilityLayer for OmniPaxosDurability {
    fn iter(&self) -> Box<dyn Iterator<Item = (TxOffset, TxData)>> {
        ...
        let log_iter = self
            .omnipaxos
            .read_entries(0..self.omnipaxos.get_decided_idx());
        let decided_entries: Vec<(TxOffset, TxData)> = log_iter
            .unwrap()
            .iter()
            .filter_map(|log_entry| Self::decode_log_entry_decided(log_entry.clone()))
            .collect();
    ...
    fn iter_starting_from_offset(
        ...
        let decided_entries: Vec<(TxOffset, TxData)> = entries_after_offset
            .iter()
            .filter_map(|log_entry| Self::decode_log_entry_decided(log_entry.clone()))
            .collect();
